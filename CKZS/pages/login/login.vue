<template>
  <view class="page">
    ...
    <!-- 品牌区 -->
    <view class="brand">
      <text class="brand-name">智能喂虾机器</text>
      <text class="brand-tagline">Smart Shrimp Feeder</text>
    </view>

    <!-- 登录卡片 — 毛玻璃 -->
    <view class="glass-card">
      <text class="card-heading">登录</text>
      ...
      <view class="guest-btn" @click="enterAsGuest">
        <text>游客模式</text>
      </view>

      <text class="switch" @click="toRegister">还没有账号？<text class="em">立即注册</text></text>
    </view>
  </view>
</template>
...
onMounted(() => {
  // 预填上次保存的账号密码
  username.value = uni.getStorageSync(SAVED_USERNAME) || '';
  password.value = uni.getStorageSync(SAVED_PASSWORD) || '';
});
...
const handleLogin = async () => {
  ...
  loading.value = true;
  try {
    const data = await http.post('/api/users/login', { username: u, password: p });
...
const toRegister = () => uni.navigateTo({ url: '/pages/login/register' });
const enterAsGuest = () => uni.reLaunch({ url: '/pages/index/index' });
</script>
