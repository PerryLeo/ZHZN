    ...
    <section class="panel">
      ...
      <form class="command-form" @submit.prevent="openConfirm">
        ...
        <div class="form-field">
          <div class="command-payload-header">
            <label>透传内容</label>
            <div class="quick-command-list" aria-label="快捷指令">
              <button v-for="command in quickCommands" :key="command" type="button" @click="form.payload = command">{{ command }}</button>
            </div>
          </div>
          <textarea v-model="form.payload" class="form-control" placeholder="请输入需要下发的透传内容，例如：$#"></textarea>
        </div>
